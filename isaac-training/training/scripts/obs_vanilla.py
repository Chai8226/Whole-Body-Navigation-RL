...
def spawn_static_obstacles(cfg, num_envs, map_range):
    ...
    # ==================== whole-body ====================
    # 将水平横梁添加到地形网格中，以便LiDAR可以检测到它们
    num_beams = int(getattr(cfg.env, "num_static_beams", 12))
    beam_len_range = tuple(getattr(cfg.env, "beam_length_range", [2.0, 6.0]))
    beam_thk_range = tuple(getattr(cfg.env, "beam_thickness_range", [0.2, 0.4]))
    beam_z_range = tuple(getattr(cfg.env, "beam_height_range", [1.0, 3.0]))
